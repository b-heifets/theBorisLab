...
### miracl_reg_clar-allen_whole_brain_iDISCO.sh -i niftis/sample2_02x_down_autofl_chan.nii.gz -o PSL -m split -s lh -v 10 -b 0
...
function usage()
{
    ...
    optional arguments:
        ...
        ###miracl_reg_clar-allen_whole_brain.sh -i niftis/sample28_Lhemi_02x_down_autofl_chan.nii.gz -o AIL -m split -s lh -v 10 -b 0 -w 1
...
if [[ -z ${ort} ]]; then
    ort=ALS
# ort changed from ARS by AUsten Casey for whole brain reg test 10/10/21
fi
...
function warpallenlbls()
{
    ...
    ifdsntexistrun ${tiflbls} "Un-padding high res tif" c3d ${unpadtif} -pad -10% -10% -o ${tiflbls} ###uncommented to make low res tif
...
# 2a) initialize registration
    ...
    if [[ "${bulb}" == 0 ]]; then
        allenref=${atlasdir}/ara/gubra/gubra_template_wo_OB_25um${side}.nii.gz

    elif [[ "${bulb}" == 1 ]]; then
        allenref=${atlasdir}/ara/gubra/gubra_template_25um${side}.nii.gz
...
# 4) Warp input CLARITY to Allen
	...
	# hres Allen
	allenhres=${atlasdir}/ara/gubra/gubra_template_10um.nii.gz
