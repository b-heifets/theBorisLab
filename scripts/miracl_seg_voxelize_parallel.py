...
def parsefn():
    ...
    parser.add_argument('-s', '--seg', type=str, help="binary segmentation tif", required=True) ### changed required to False DRR when commenting out bin part, and back to true later
...
ncpus = int(cpuload * cpus)  # 95% of cores used by default
...
def main(args):
    ...
    outvox = '%s/voxelized_seg_%s.tif' % (segdir, type) ##DRR changed .tiff to .tif
    ...
    ###segbasebin = base.replace("seg", "seg_bin") ###seg bin files commented out by DRR since they are not needed as we used binary input
    ###segbin = segdir + "/" + segbasebin

    ###outvoxbin = '%s/voxelized_seg_%s_bin.tif' % (segdir, type) ##DRR changed .tiff to .tif
