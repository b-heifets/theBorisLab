...
/usr/local/miracl/depends/Fiji.app/ImageJ-linux64 --ij2 -macro ABAseg_75sliceSubstacks_RH "$Seg"#"$ABA"

#3D object count (cd ABAseg_stacks directory -> . activate miracl -> 3dc.sh 
#saves to sample folder and ABAseg_stacks folder

cd "$SegDir"/ABAseg_stacks_75slices
...
find "all.csv" | xargs cut -d, -f16,37,39,40,42,43,45 > Seg_3Dcount_75slices_RH.csv #extracts columns from all.csv 
...
StacksDir="$PWD"
mv Seg_3Dcount_75slices_RH.csv ../
cd ../
ParentFolder=${PWD##*/}
for f in Seg_3Dcount_75slices_RH.csv ;  do mv "$f" "$(basename "$(pwd)")"_"$f" ;  done #Append parent folder name (e.g., seg_ilastik_DR_)
mv "$ParentFolder"_Seg_3Dcount_75slices_RH.csv ../
cd ../
GrantParentDir=$(pwd)
GrandParentFolder=${PWD##*/}
for f in "$ParentFolder"_Seg_3Dcount_75slices_RH.csv ;  do mv "$f" "$(basename "$(pwd)")"_"$f" ;  done #Append grandparent folder name (e.g., KNK_sample2_)
Filename="$GrandParentFolder"_"$ParentFolder"_Seg_3Dcount_75slices_RH.csv
