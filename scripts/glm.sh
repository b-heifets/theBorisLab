...
mkdir GLM_stats/ ; cp /usr/local/miracl/atlases/ara/gubra/gubra_template_25um_thr30_bin.nii.gz ./GLM_stats/ ; cp /usr/local/miracl/atlases/ara/gubra/gubra_ano_split_25um_uthr40000.nii.gz ./GLM_stats/
...
fslmaths s_thr_bin_sum_gubra_space.nii.gz -add gubra_template_25um_thr30_bin.nii.gz sum_activity_template_mask.nii.gz

fslmaths sum_activity_template_mask.nii.gz -thr 2 thr2_sum_activity_template_mask.nii.gz

fslmaths thr2_sum_activity_template_mask.nii.gz -bin GLM_mask.nii.gz
...
randomise_parallel -i all_norm_s50.nii.gz -m GLM_mask.nii.gz -o "$GLMFolderName"_"$group1"-"$group1_N"_"$group2"-"$group2_N" -d design.mat -t design.con -T -n 5000
