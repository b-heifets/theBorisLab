...
/usr/local/miracl/depends/Fiji.app/ImageJ-linux64 --ij2 -macro ABAseg_75sliceSubstacks "$Seg"#"$ABA"
...
find "all.csv" | xargs cut -d, -f16,37,39,40,42,43,45 > Seg_3Dcount_75slices.csv #extracts columns from all.csv 
...
StacksDir="$PWD"
mv Seg_3Dcount_75slices.csv ../
cd ../
ParentFolder=${PWD##*/}
for f in Seg_3Dcount_75slices.csv ;  do mv "$f" "$(basename "$(pwd)")"_"$f" ;  done #Append parent folder name (e.g., seg_ilastik_DR_)
mv "$ParentFolder"_Seg_3Dcount_75slices.csv ../
cd ../
GrantParentDir=$(pwd)
GrandParentFolder=${PWD##*/}
for f in "$ParentFolder"_Seg_3Dcount_75slices.csv ;  do mv "$f" "$(basename "$(pwd)")"_"$f" ;  done #Append grandparent folder name (e.g., KNK_sample2_)
Filename="$GrandParentFolder"_"$ParentFolder"_Seg_3Dcount_75slices.csv
