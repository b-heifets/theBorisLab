...
tstat2=$(ls ./*_tstat2.nii.gz| tail -1) #input clarity nii
...
if [[ ! -f "$outfile" ]]; 
	then
		...
		cluster -i "$base" -t ${thr%?} -o cluster_index_tstat1_percentile"$tstat_percentile"_thr${thr%?} --osize=cluster_size_tstat2_percentile"$tstat_percentile"_thr${thr%?} > cluster_info_tstat2_percentile"$tstat_percentile"_thr${thr%?}.txt
